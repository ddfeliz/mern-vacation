  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      ...
      if (checkResponse.data.exists) {
        // setOpenVerify(true);
        toast.warning('Un correcteur existe déjà avec ce CIN!');
        setLoading(false);
        return; // Arrêter la soumission
      } 
      else if (formData.cin.length !== 12) {
        // setOpenCIN(true);
        toast.warning(`Le CIN que tu as saisi doit être 12 chiffre! Veuillez verifier bien votre CIN.`);
        setLoading(false);
        return;
      } 
      else {
                  ...
                  <input
                    ...
                    onBlur={() => {
                      ...
                      if (formData.cin.length !== 12) {
                        // setOpenCIN(true);
                        toast.warning(`Le CIN que tu as saisi doit être 12 chiffre!
                          Veuillez verifier bien votre CIN.`);
                        return;
                      }
                    }}
                    minLength={12}
